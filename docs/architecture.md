...
- `load_profiles()` → `list[dict]` — reads JSON, returns `[]` on any error
- `save_profile(name, trigger, sink, source, bt_profile='')` — inserts or updates in-place
- `delete_profile(trigger)` → `bool`
...
- `check_and_route_device(node_name, monitor=None)` — loads profiles, fires routing actions, returns `bool`
...
Also exports `get_audio_nodes_sync(callback=None, timeout_ms=2000)`:
- Uses `wpctl status` + `wpctl inspect <id>` for each node (no WirePlumber GObject needed)
- Returns `list[dict]` with `name` (PipeWire technical name, e.g. `bluez_output.XX_XX_XX_XX_XX_XX.a2dp-sink`), `description` (user-facing label, e.g. `JLab GO Air Pop`), `media_class` (`Audio/Sink`, `Audio/Source`, `Audio/Duplex`)
...
3. Runs `GLib.MainLoop` indefinitely
...
### `window.py`
`AutowireWindow` — shows the profile list from `profiles.json`.

- `refresh_profiles()` — clears and rebuilds `Adw.PreferencesGroup` from disk
- `_build_profile_row(profile)` — `Adw.ActionRow` with Edit + Delete buttons
...
- Device lists loaded via `threading.Thread` → `get_audio_nodes_sync()` → `GLib.idle_add` back to main thread
- `_on_devices_loaded()` populates `Gtk.StringList` models for trigger/sink/source rows
- `_validate()` enables Save only when name is non-empty AND trigger is selected
- `_on_save()` calls `config_mgr.save_profile()` → emits `profile-saved` → closes
    ...
    └─► _load_devices_async() spawns threading.Thread
            └─► get_audio_nodes_sync() via wpctl
                    └─► results → GLib.idle_add → _on_devices_loaded()
                            └─► ComboRow models set, dialog is ready

User clicks Save
    └─► config_mgr.save_profile() writes profiles.json (atomic)
...
- **`wpctl inspect`** returns Device-type objects (no `node.name`) for hardware cards (e.g. ALSA card 59). Only Node-type objects (IDs 42, 43, 48…) have `node.name`. The UI displays the description string (e.g. `JLab GO Air Pop`) which is human-readable.
